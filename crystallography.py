...
def computeKic(aold,anew,materialprops):
    ...
    return np.sqrt(2*gammasurf/A), 2*gammasurf, A
...
def computeKie(aold,anew,burgersvector,dislocationtype,materialprops):
    ...
    theta = Mmath.computeAngle(crackplane,slipplane)
    phi = Mmath.computeAngle(burgersvector,slipplanet)
...
def rotationTensor(aold,anew):
    return np.einsum('ij,kj',Mmath.normalizeVecAll(aold,1),Mmath.normalizeVecAll(anew,1))
...
def planeStrainComplianceVoigt(matrix,symmoption=True):
    ...
    if symmoption: # eliminate shear-normal coupling terms
        matrixnew[0,2], matrixnew[1,2], matrixnew[2,0], matrixnew[2,1] = 4*[0]
...
def hexVoigt(dict):
    ...
    tensor = np.zeros([6,6])
    tensor[0,0], tensor[1,1] = [dict['11']]*2
    tensor[2,2] = dict['33']
    tensor[0,2], tensor[2,0], tensor[1,2], tensor[2,1] = [dict['13']]*4
    tensor[3,3], tensor[4,4] = [dict['44']]*2
    tensor[5,5] = dict['66']
    tensor[0,1], tensor[1,0] = [dict['11'] - 2*dict['66']]*2
    return tensor
...
def voigtToTensor(matrix):
    ...
    for i in range(6):
        ...
        for j in range(6):
            ...
            tensor[l,m,n,p], tensor[m,l,n,p], tensor[l,m,p,n], tensor[m,l,p,n] = [matrix[i,j]]*4
    return tensor
...
def voigtToTensorIndex(val):
    if val < 3:
        ...
        return Mmath.setDiff(range(3),set([val-3]))
        
def tensorToVoigtIndex(vals):
    if vals[0] == vals[1]:
        return vals[0]
    else:
        sol = Mmath.setDiff(range(3),set(vals))
        return sol[0] + 3
...
def getMaterialProps(materialname): # units - energy [J/m^2]; stiffness [N/m^2]
    ...
    nickelprops = {'elastic': nickelelastic,
